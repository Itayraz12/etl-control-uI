...
filters:
  ...
  it('sends deploy request params and configurationYaml when saving and deploying', async () => {
    ...
    expect(mockDeployFromYaml.mock.calls[0][0].configurationYaml).not.toContain('\n  entity:')
    expect(mockDeployFromYaml.mock.calls[0][0].configurationYaml).toContain(`location: "${DEFAULT_METADATA_LOCATION}"`)
    ...
    expect(mockDeployFromYaml.mock.calls[0][0].configurationYaml).toContain('output:\n  mapping:')
    expect(mockDeployFromYaml.mock.calls[0][0].configurationYaml).toContain('    - inName: sku\n      outName: sku\n      type: string')
    expect(mockDeployFromYaml.mock.calls[0][0].configurationYaml).toContain('  kafka:\n    brokers: production\n    topic: catalog-sink')
    expect(mockDeployFromYaml.mock.calls[0][0].configurationYaml).toContain(`  ${SAKNAY_YAML_SECTION_KEY}:\n    ${SAKNAY_TOPIC_YAML_KEY}:`)
